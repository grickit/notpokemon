//# CLEAN but perhaps not the best way to do things
...
// ----- CLASS: OutputBox {
  function OutputBox(id, title, default_delay) {
    ...
    this.default_scroll_delay = (default_delay == undefined)? 60 : default_delay;
    this.scroll_delay = this.default_scroll_delay;

    // Methods
    this.write = function(message,br) { this.messages.push({type:'normal', message: message, br: br}); }

    this.scrollingWrite = function(message,br) { this.messages.push({type:'scrolling', message: message, br: br}); }

    this.clear = function(message) { this.messages.push({type:'clear'}); }

    this.wait = function(milliseconds) { this.messages.push({type:'wait', message: milliseconds}); }

    this.changeDelay = function(milliseconds) { this.messages.push({type:'delay change', message: milliseconds}); }

    this.resetDelay = function() { this.messages.push({type:'delay change', message: this.default_scroll_delay}); }
    ...
    this.changeDelayNow = function(milliseconds) { this.scroll_delay = milliseconds; }
